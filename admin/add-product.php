<?php
  ...
  if (isset($_POST["add_product"])) {
    ...
    if (empty($fileTmpName)) {
      array_push($invalid, "Image should not be empty!");
    } if (empty($name)) {
      ...
      $verifyProduct = $shopFacade->verifyProduct($name);

      if ($verifyProduct > 0) {
        array_push($invalid, "Product already exist!");
      } elseif ($fileError === 0) {
            ...
            $addProduct = $shopFacade->addProduct($image, $name, $price, $link);
						if ($addProduct) {
							array_push($success, "Product added successfully!");
						} else {
							array_push($invalid, "Product not added!");
						}
					} else {
            ...
            <form class="form-horizontal" action="add-product.php" method="post" enctype="multipart/form-data">
              <div class="card-body">
                ...
                <div class="form-group row">
                  ...
                  <div class="col-sm-10">
                    <input type="text" class="form-control" id="name" placeholder="Name Here" name="name">
                  </div>
                </div>
                <div class="form-group row">
                ...
                <div class="form-group row">
                  ...
                  <div class="col-sm-10">
                    <textarea class="w-100 p-2" id="link" placeholder="Link Here" name="link" style="height: 130px"></textarea>
                  </div>
                </div>
              </div>
